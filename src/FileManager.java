...
public class FileManager {
        ...
        public void saveData(UserManager userManager, FriendGraph friendGraph, InterestManager interestManager,
                ...
                FileWriter writer = new FiileWriter(filePath);
                ...
                for(int user = 0; user < numUsers; user++){
                        ...
                        int numFriends = userManager.getAllUsers().get(user).getFriendsBST().getLength();
                        ...
                        int numInterests = userManager.getAllUsers().get(user).getInterestsList().getLength();

                        System.out.println(numInterests + "\n");

                        writer.write(numInterests +"\n")

                        LinkedList listOfInterests = userManager.getAllUsers().get(user).getInterestsList();

                        for(int interest = 0; interest < numInterests; interest++){

                                listOfInterests.advanceIteratorToIndex(interest);
 
                                String inter = listOfInterests.getIterator();
